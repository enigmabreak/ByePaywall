...
ext_api.runtime.onMessage.addListener(function (message, sender) {
  ...
  if (message.request === 'getExtSrc' && message.data) {
    message.data.html = '';
    function processFetchedHtml(html, message, final_url) {
      let recursive;
      if (message.data.url.startsWith('https://archive.')) {
        if (final_url.includes('/https')) {
    ...
    function fetchViaTab(url, message) {
      ext_api.tabs.create({ url: url, active: true }, function (tab) {
        let tabId = tab.id;
        let timeoutId = setTimeout(() => {
          ext_api.tabs.onUpdated.removeListener(tabUpdateListener);
          ext_api.tabs.update(sender.tab.id, {active: true}).catch(() => {});
          ext_api.tabs.remove(tabId).catch(() => {});
          ...
          if (updatedTabId === tabId && changeInfo.status === 'complete') {
            if (ext_manifest_version === 3) {
              ext_api.scripting.executeScript({
                ...
                let html = results && results[0] ? results[0].result : '';
                let isCaptcha = html.includes('Why do I have to complete a CAPTCHA?') || html.includes('Attention Required! | Cloudflare') || html.includes('Just a moment...');
                let isSnapshot = !isCaptcha && !html.match(/<title>\s*archive\.\w{2,5}\s*<\/title>/i);
                ...
                if (html.includes('<div class="TEXT-BLOCK"')) {
                  ...
                  ext_api.tabs.update(sender.tab.id, {active: true}).catch(() => {});
                  ext_api.tabs.remove(tabId).catch(() => {});
                  processFetchedHtml(html, message, updatedTab.url);
                } else if (isSnapshot) {
                  ext_api.tabs.onUpdated.removeListener(tabUpdateListener);
                  clearTimeout(timeoutId);
                  ext_api.tabs.update(sender.tab.id, {active: true}).catch(() => {});
              ...
              ext_api.tabs.executeScript(tabId, {
                ...
                let html = results && results[0] ? results[0] : '';
                let isCaptcha = html.includes('Why do I have to complete a CAPTCHA?') || html.includes('Attention Required! | Cloudflare') || html.includes('Just a moment...');
                let isSnapshot = !isCaptcha && !html.match(/<title>\s*archive\.\w{2,5}\s*<\/title>/i);
        ...
        ext_api.tabs.onUpdated.addListener(tabUpdateListener);
      });
    }
    ...
    function getArticleSrc(message) {
      ...
      .then(response => {
        if (response.ok) {
          response.text().then(html => {
            processFetchedHtml(html, message, response.url);
          });
        } else if (response.status === 403 || response.status === 429) {
          fetchViaTab(url_src, message);
        } else {
          message.data.blocked = true;
          sendArticleSrc(sender.tab.id, message);
        }
