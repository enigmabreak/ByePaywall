...
cs_default = function (bg2csData = '') {
  ...
  if (!(csDone || csDoneOnce)) {
    ...
    if (matchDomain('substack.com') || document.querySelector('meta[name="subdomain"][content]')) {
      let url = window.location.href;
      getArchive(url, 'div.paywall, div.paywall-content, div.subscribe-widget, div[class*="paywall"]', '', 'div.available-content, div.post-content, article', '', 'div.available-content, div.post-content, article', 'body');
      return;
    }
    ...
    if (matchDomain('medium.com') || matchDomain(medium_custom_domains) || document.querySelector('head > link[href*=".medium.com/"]')) {
      ...
      if (matchDomain('autocar.co.uk')) {
        ...
        func_post = function () {
          ...
          // Apply layout fixes for immersive/Big Read articles on all platforms (not just mobile)
          ...
          if (topper) {
            topper.style.cssText = 'max-width: 700px; margin: 0 auto; padding: 20px; background: none;';
            topper.querySelectorAll('[style*="background"]').forEach(e => {
              if (e.tagName !== 'IMG' && !e.querySelector('img'))
                e.style.background = 'none';
            });
          }
          // Fix comment section: constrain width and reset archive styling
          document.querySelectorAll('div[class*="comment"], section[class*="comment"], div[data-trackable="comments"]').forEach(e => {
            e.style.cssText = 'max-width: 700px; margin: 0 auto; padding: 20px; background: #FFF1E5; color: #33302E;';
          });
          // Reset any teal/green background colors from archive on the page
          document.querySelectorAll('[style*="background-color: rgb(0, 128, 128)"], [style*="background-color:rgb(0, 128, 128)"], [style*="background: rgb(0, 128, 128)"]').forEach(e => {
            e.style.backgroundColor = '#FFF1E5';
            e.style.color = '#33302E';
          });
          // Fix inline images in article body to not overflow
          document.querySelectorAll('div[style*="article-body"] figure, div[class*="article-body"] figure').forEach(e => {
            e.style.cssText = 'max-width: 100%; margin: 20px 0;';
          });
          // Hide archive direction links at the top (func_post only runs on successful fetch)
          document.querySelectorAll('div#bpc_archive').forEach(e => e.style.display = 'none');
          // Also hide the parent wrapper div that creates a 50px gap
          document.querySelectorAll('div#bpc_archive').forEach(e => {
            if (e.parentNode && !e.parentNode.id && e.parentNode.style.cssText.includes('margin')) {
              e.parentNode.style.display = 'none';
            }
          });
          // Fix comment section: collapse the huge empty textarea and comment input box
          document.querySelectorAll('textarea').forEach(e => {
            e.style.cssText = 'height: 80px; width: 100%; max-width: 700px; resize: vertical;';
          });
          // Constrain the comment guidelines and sign-in area
          addStyle('div[class*="comment"] { max-width: 700px !important; margin-left: auto !important; margin-right: auto !important; } ' +
            'div[class*="comment"] textarea { height: 80px !important; } ' +
            'div[data-trackable="comments"] { max-width: 700px !important; margin-left: auto !important; margin-right: auto !important; }', 'bpc_ft_fix');
          // Try to restore embedded interactive content (iframes stripped by archive)
          document.querySelectorAll('iframe[data-src]:not([src]), iframe[old-src]:not([src])').forEach(e => {
            let src = e.getAttribute('data-src') || e.getAttribute('old-src');
            if (src) {
              e.src = src;
              e.style.cssText = 'width: 100%; height: 600px; border: none; margin: 20px 0;';
            }
          });
          // Restore images with data-src (lazy-loaded images stripped by archive)
          document.querySelectorAll('img[data-src]:not([src]), img[old-src]:not([src])').forEach(e => {
            let src = e.getAttribute('data-src') || e.getAttribute('old-src');
            if (src) e.src = src;
          });
    ...
    // Recreate social sharing sidebar for archive-fetched articles
    let articleContent = document.querySelector('div[style*="article-body"], div[class*="article-body"]');
    if (articleContent && !document.querySelector('.bpc-share-sidebar')) {
      let pageUrl = encodeURIComponent(window.location.href);
      let pageTitle = encodeURIComponent(document.title);
      let shareBar = document.createElement('div');
      shareBar.className = 'bpc-share-sidebar';
      shareBar.style.cssText = 'position: sticky; top: 100px; float: left; margin-left: -60px; display: flex; flex-direction: column; gap: 12px; z-index: 100;';
      let shares = [
        {label: '𝕏', url: 'https://twitter.com/intent/tweet?url=' + pageUrl + '&text=' + pageTitle},
        {label: 'f', url: 'https://www.facebook.com/sharer/sharer.php?u=' + pageUrl},
        {label: 'in', url: 'https://www.linkedin.com/shareArticle?mini=true&url=' + pageUrl + '&title=' + pageTitle},
      ];
      for (let s of shares) {
        ...
        btn.style.cssText = 'display: flex; align-items: center; justify-content: center; width: 36px; height: 36px; border: 1px solid #ccc; border-radius: 50%; text-decoration: none; color: #33302E; font-size: 14px; font-weight: bold; background: #fff;';
        shareBar.appendChild(btn);
      }
      articleContent.style.position = 'relative';
      articleContent.prepend(shareBar);
    }
    let url = window.location.href;
    if (url.includes('/content/')) {
        let paywall_sel = 'div#barrier-page, .o-barrier, .o-barrier-page, [data-trackable="barrier"], [data-trackable="paywall"]';
        let content_sel = 'div.n-layout__row--content, article#site-content, main#site-content, .article-content';
        getArchive(url, paywall_sel, '', content_sel, '', 'div[style*="article-body"], article#site-content, main#site-content', 'body');
    }
